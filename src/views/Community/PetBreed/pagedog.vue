    ...
    >
      <template v-slot:default="props">
        <v-row class="champ">
          ...
          >
            <v-card height="220" min-width="160" width="220" align="center" justify="center">
<v-row>
  <v-col>
     <v-img @click="onClickImage(item.id)"
                  :src="
...
<v-row>
<v-container >
              <v-col class="Hide-text" align="center" justify="start">{{ item.pet_title_th  }} ({{ item.pet_title }})</v-col>
...
export default {
  ...
  async mounted () {
    ...
    result.data.list_breed_pets.forEach(DOG => {
      this.DOGData.push(DOG)
    })
  },

  computed: {
    numberOfPages () {
      return Math.ceil(this.DOGData.length / this.itemsPerPage)
    }
  },
  ...
  methods: {
    ...
    onClickImage (dogbreedID) {
      this.DOGbreedID = dogbreedID
      this.$ls.set('dogbreedID', JSON.stringify(this.DOGbreedID))
      this.$router.push({ name: 'profiledog' })
    }
  }
}
</script>
